...
class dianpingFoodEngine:

    def __init__(self) -> None:
        super().__init__()
        self.down = dianpingFoodDownloader()
        self.spider = dianpingFoodSpider()
        self.pipe = dianpingFoodPipeline()
    ...
    def get_category(self, city_id):
        url = dianpingFoodSetting.CATEGORY_URL % city_id
...
class dianpingFoodDownloader:

    def __init__(self) -> None:
        super().__init__()
        self.session = requests.session()
        self.cookies = dianpingFoodSetting.COOKIES
    ...
    def do_get_requests(self, *args):
        ...
        while retry > 0:
            try:
                ...
                time.sleep(dianpingFoodSetting.DELAY)
    ...
    def get_category(self, url):
        headers = dianpingFoodSetting.HEADERS
    ...
    def get_info(self, id):
        headers = dianpingFoodSetting.HEADERS_JSON
        url = dianpingFoodSetting.INFO_URL
        params = dianpingFoodSetting.PARAMS
        params['shopId'] = id
    ...
    def get_comment(self, shopid, page):
        url = dianpingFoodSetting.CMT_URL % (shopid, page)
        headers = dianpingFoodSetting.HEADERS
        html = self.do_get_requests(url, headers)
        return html


class dianpingFoodSpider:
    def get_category(self, html):
        selector = etree.HTML(html)
        cate = dianpingFoodSetting.FOOD_CATEGORY
        ...
        #             count = i.xpath('@data-click-name')[0] if i.xpath('@data-click-name') else ''
        #             data.append((name, count))
        #             break
        cons = selector.xpath('//div[@id="classfy"]/a')
        data = []
        for each in cons:
            name = each.xpath('span/text()')[0]
            count = each.xpath('@href')[0].split('/')[-1]
            data.append((name, count))
        return data
...
class dianpingFoodPipeline:
    def __init__(self):
        self.blank = dianpingFoodSetting.BLANK
        self.code = dianpingFoodSetting.ENCODING
    ...
    def save_category_list(self, cate_list):
        ...
        text = ''
        f = open(config.CATEGORY_FOOD, 'w', encoding=self.code)
        for i in cate_list:
    ...
    def construct_url(self):
        urls = []
        url = dianpingFoodSetting.SHOP_URL
        area = config.AREA_FILE
        food_cate = config.CATEGORY_FOOD
        for i in open(area, 'r', encoding=self.code):
            for t in open(food_cate, 'r', encoding=self.code):
    ...
    def save_shop_list(self, data, info, restaurant_set):
        ...
        with open(config.RESTAURANT_LIST % config.PROVINCE , 'a', encoding=self.code) as f:
            f.write(content)

    def get_shop_list(self):
        if os.path.exists(config.RESTAURANT_LIST % config.PROVINCE):
            shop_list = (
                i.strip().split(
    ...
    def get_shop_list_set(self):
        if os.path.exists(config.EX_RESTAURANT_ID_LIST):
            shop_list_set = set(str(i.strip()) for i in open(
                config.EX_RESTAURANT_ID_LIST, 'r', encoding=self.code))
        else:
            open(config.EX_RESTAURANT_ID_LIST, 'w+', encoding=self.code)
            shop_list_set = set(str(i.strip()) for i in open(
                config.EX_RESTAURANT_ID_LIST, 'r', encoding=self.code))
        return shop_list_set

    def save_shop_info(self, data, info, url):
        shop = config.RESTAURANT_DATA
        shop['中文全称'] = info[1]
        shop['所属地区'] = info[8]
        shop['地址'] = data[0]
        shop['地理位置'] = data[2]
        ...
        with open(config.RESTAURANT_INFO % config.PROVINCE, 'a', encoding=self.code) as f:
            ...
            text = self.blank.join([shop[i].replace('&', '').replace('\n', '').replace(
                '\r', '').replace('None', '') for i in config.RESTAURANT_DATA_L])
            f.write(text + '\n')
        # 保存这家店的ID
        with open(config.EX_RESTAURANT_ID_LIST, 'a', encoding=self.code) as p:
            p.write(info[0] + '\n')
...
class dianpingFoodSetting:
    ...
    CATEGORY_URL = 'http://www.dianping.com/search/category/%s/10/g110'

    # 四个参数，依旧是cityid， 菜品分类， 行政区
    SHOP_URL = 'http://www.dianping.com/search/category/%s/10/%s%sp'
...
class dianpingSchedule:
    ...
    def load_2_hdfs(self, min_date, max_date):
        ...
        shop_list = config.RESTAURANT_LIST % config.PROVINCE
