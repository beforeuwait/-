...
class dianpingShoppingEngine:

    def __init__(self) -> None:
        super().__init__()
        self.down = dianpingShoppingDownloader()
        self.spider = dianpingShoppingSpider()
        self.pipe = dianpingShoppingPipeline()
    ...
    def get_category(self, city_id):
        url = dianpingShoppingSetting.CATEGORY_URL % city_id
...
class dianpingShoppingDownloader:

    def __init__(self) -> None:
        super().__init__()
        self.session = requests.session()
        self.cookies = dianpingShoppingSetting.COOKIES
    ...
    def do_get_requests(self, *args):
        ...
        while retry > 0:
            try:
                ...
                time.sleep(dianpingShoppingSetting.DELAY)
                ...
                if response.status_code == 200:
                    ...
                    html = response.content.decode(
                        dianpingShoppingSetting.ENCODING)
                    break
    ...
    def get_category(self, url):
        headers = dianpingShoppingSetting.HEADERS
    ...
    def get_info(self, id):
        headers = dianpingShoppingSetting.HEADERS_JSON
        url = dianpingShoppingSetting.INFO_URL
        params = dianpingShoppingSetting.PARAMS
        params['shopId'] = id
    ...
    def get_comment(self, shopid, page):
        url = dianpingShoppingSetting.CMT_URL % (shopid, page)
        headers = dianpingShoppingSetting.HEADERS
        html = self.do_get_requests(url, headers)
        return html


class dianpingShoppingSpider:
    def get_category(self, html):
        selector = etree.HTML(html)
        cate = dianpingShoppingSetting.SHOPPING_CATEGORY
        cons = selector.xpath('//div[@class="groups"]/div[@class="group"]')
        data = []
        for each in cons:
            if each.xpath('div[@class="sec-title"]/span/text()')[0] == cate:
        ...
        try:
            ...
            if cons:
                for each in cons:
                    ...
                    socer = ','.join(each.xpath('div[@class="content"]/div[@class="user-info"]/div[@class="comment-rst"]/span/text()'))\
                        if each.xpath(
                    ...
                    imgs = ''
                    try:
                        if each.xpath('div[@class="content"]/div[@class="shop-photo"]/ul/li'):
                            imgs = ','.join(
...
class dianpingShoppingPipeline:
    def __init__(self):
        self.blank = dianpingShoppingSetting.BLANK
        self.code = dianpingShoppingSetting.ENCODING
    ...
    def save_category_list(self, cate_list):
        ...
        text = ''
        f = open(config.CATEGORY_SHOPPING, 'w', encoding=self.code)
        for i in cate_list:
    ...
    def construct_url(self):
        urls = []
        url = dianpingShoppingSetting.SHOP_URL
        area = config.AREA_FILE
        shopping_cate = config.CATEGORY_SHOPPING
        for i in open(area, 'r', encoding=self.code):
            for t in open(shopping_cate, 'r', encoding=self.code):
    ...
    def save_shop_list(self, data, info, restaurant_set):
        ...
        with open(config.SHOPPING_LIST % config.PROVINCE, 'a', encoding=self.code) as f:
            f.write(content)

    def get_shop_list(self):
        if os.path.exists(config.SHOPPING_LIST % config.PROVINCE):
            shop_list = (
                i.strip().split(
            ...
            open(config.SHOPPING_LIST % config.PROVINCE, 'w+', encoding=self.code)
            shop_list = (
                i.strip().split(
                    self.blank) for i in open(
                config.SHOPPING_LIST % config.PROVINCE,
    ...
    def get_shop_list_set(self):
        if os.path.exists(config.EX_SHOPPING_ID_LIST):
            shop_list_set = set(str(i.strip()) for i in open(
                config.EX_SHOPPING_ID_LIST, 'r', encoding=self.code))
        else:
            open(config.EX_SHOPPING_ID_LIST, 'w+', encoding=self.code)
            shop_list_set = set(str(i.strip()) for i in open(
                config.EX_SHOPPING_ID_LIST, 'r', encoding=self.code))
        return shop_list_set

    def save_shop_info(self, data, info, url):
        shop = config.SHOPPING_DATA
        shop['中文全称'] = info[1]
        shop['所属地区'] = info[8]
        shop['地址'] = data[0]
        shop['地理位置'] = data[2]
        ...
        with open(config.SHOPPING_INFO % config.PROVINCE, 'a', encoding=self.code) as f:
            ...
            text = self.blank.join([shop[i].replace('&', '').replace('\n', '').replace(
                '\r', '').replace('None', '') for i in config.SHOPPING_DATA_L])
            f.write(text + '\n')
        # 保存这家店的ID
        with open(config.EX_SHOPPING_ID_LIST, 'a', encoding=self.code) as p:
            p.write(info[0] + '\n')
...
class dianpingShoppingSetting:
    ...
    CATEGORY_URL = 'http://www.dianping.com/search/category/%s/50/'

    # 四个参数，依旧是cityid， 菜品分类， 行政区
    SHOP_URL = 'http://www.dianping.com/search/category/%s/50/%s%sp'
...
class dianpingSchedule:
    ...
    def execute(self, min_date, max_date):
        ...
        with open(config.MIN_DATE_FILE_SHOP, 'w', encoding='utf8') as f:
    ...
    def load_2_hdfs(self, min_date, max_date):
        ...
        shop_list = config.SHOPPING_LIST % config.PROVINCE
