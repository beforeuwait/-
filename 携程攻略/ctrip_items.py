...
class CtripItemsEngine(object):
    ...
    def __init__(self):
        self.down = CtripItemsDownloader()
        self.spider = CtripItemsSpider()
        self.pipe = CtripItemsPipeline()
    ...
    def shop_list(self, choice):
        ...
        self.clear_path(setting.shop_list[choice])
        city_list_clear = self.city_list(setting.city_list[setting.path], choice)
        for city in city_list_clear:
            city = city.split(setting.blank)
        ...
        if choice == 'shopping':
            ...
            for city in city_list:
                ...
                if info[-2] not in city_set:
                    ...
                    info[6] = str(code) + '00'
                    new_city_list.append(setting.blank.join(info))
        else:
    ...
    def shop_list_logic(self, city, choice):
        ...
        while next_page:
            ...
            shop_list_data = setting.shop_list_data
    ...
    def shop_info_pid(self, choice):
        ...
        shop_already_exist = set(i.strip().split(setting.blank)[0]
                                 for i in open(setting.shop_exists[choice], 'r', encoding=setting.encode))

        shop_list = (i.strip().split(setting.blank)
                     for i in open(setting.shop_list[choice], 'r', encoding=setting.encode))
    ...
    def shop_info_pid_logic(self, shop, choice):
        ...
        shop_info_data = setting.shop_info_data
        # 记录请求过程
        self.recording_response(response)
        if response['response'] is not 'bad_requests':
    ...
    def shop_comment(self, start, end, choice):
        """这里作为获取每个店铺的评论模块"""
        # 每次开始前都将把cmt目录清空
        self.clear_path(setting.cmt_txt[choice])
        shop_list = (i.strip().split(setting.blank)for i in open(
            setting.shop_exists[choice], 'r', encoding=setting.encode))
        for shop in shop_list:
            if choice == 'shopping':
...
class CtripItemsDownloader(object):
        ...
        while retry > 0:
            ...
            try:
                if len(args) == 2:
                    ...
                    res = requests.get(args[0], headers=args[1], params=args[2], proxies=setting.proxy, timeout=30)
                # 这里暂时只对 2xx和4xx做处理
    ...
    def shop_list(self, city_id, page, choice):
        url = setting.shop_list_url[choice] % (city_id, page)
        headers = setting.headers
        response = self.do_get_requests(url, headers)
        return response

    def shop_list_area(self, city_id, area_id, page, choice):
        url = setting.shop_list_url_area[choice] % (city_id, area_id, page)
        headers = setting.headers
        response = self.do_get_requests(url, headers)
        return response
    ...
    def shop_comment(self, **kwargs):
        ...
        data = setting.comment_data[kwargs.get('choice')]
...
class CtripItemsSpider(object):
    ...
    def shop_info_pid(self, html, shop_info, choice):
        ...
        if not choice == 'shopping':
            ...
            # 解析规则
            parse = setting.shop_info_pid_parse
            data = []
...
class CtripItemsPipeline(object):
    ...
    def save_shop_list(self, info, data, choice):
        ...
        for each in data:
            ...
            text += re.sub('\n|\r| ', '', context + setting.blank + setting.blank.join(each)) + '\n'
        with open(setting.shop_list[choice], 'a', encoding=setting.encode) as f:
            f.write(text)
        return

    def save_restaurant_info_pid(self, data, shop_info, choice):
        info = setting.rest_dict
        info_l = setting.rest_dict_l
        ...
        text = re.sub('\n|\r| ', '', setting.blank.join(text_list)) + '\n'
        text_ex = setting.blank.join([shop_info[8], data[2],
                                      re.findall('(.*?)\d', shop_info[-1].split('/')[2])[0]]) + '\n'
        with open(setting.shop_info[choice], 'a', encoding=setting.encode) as f:
            f.write(text)
        with open(setting.shop_exists[choice], 'a', encoding=setting.encode) as f:
            f.write(text_ex)
    ...
    def save_shopping_info_pid(self, data, shop_info, choice):
        ...
        text_ex = setting.blank.join([shop_info[-5], data[1], data[0], data[2]]) + '\n'
        with open(setting.shop_info[choice], 'a', encoding=setting.encode) as f:
            f.write(text)
    ...
    def save_shop_cmt(self, cmt_list, shop_id, start, end, choice):
        ...
        for each in cmt_list:
            if each[-2] >= start and each[-2] <= end:
                ...
                result = False
        with open(setting.cmt_txt[choice], 'a', encoding=setting.encode) as f:
            f.write(text)

        with open(setting.cmt_txt_h[choice], 'a', encoding=setting.encode) as f:
            f.write(text)
        return result
